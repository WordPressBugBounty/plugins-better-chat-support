...
namespace ThemeAtelier\BetterChatSupport\Admin\Framework\Classes;

if (! class_exists('BetterChatSupport')) {
  class BetterChatSupport
  {

    // Default constants
    public static $premium  = true;
    public static $version  = BETTER_CHAT_SUPPORT_VERSION;
    public static $dir      = '';
    public static $url      = '';
    public static $css      = '';
    public static $file     = '';
    ...
    public function __construct()
    {
      ...
      // Init action
      do_action('BetterChatSupport_init');

      add_action('after_setup_theme', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'setup'));
      add_action('init', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'setup'));
      add_action('switch_theme', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'setup'));
      add_action('admin_enqueue_scripts', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'add_admin_enqueue_scripts'));
      add_action('wp_enqueue_scripts', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'add_typography_enqueue_styles'), 80);
      add_action('wp_head', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'add_custom_css'), 80);
      add_filter('admin_body_class', array('ThemeAtelier\BetterChatSupport\Admin\Framework\Classes\BetterChatSupport', 'add_admin_body_class'));
    }
    ...
    public static function include_plugin_file($file, $load = true)
    {
      ...
      $path     = '';
      $file     = ltrim($file, '/');
      $override = apply_filters('BetterChatSupport_override', 'better-chat-support-override');
    ...
    public static function includes()
    {
      ...
      if (! empty($fields)) {
        foreach ($fields as $field) {
          if (! class_exists('BetterChatSupport_Field_' . $field) && class_exists('BetterChatSupport_Fields')) {
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      if (! self::$enqueue) {
        ...
        if ($wpscreen->id === 'tools_page_better-chat-support-welcome') {
          self::$enqueue = true;
        }
      }

      if (! apply_filters('BetterChatSupport_enqueue_assets', self::$enqueue)) {
        return;
      }
      ...
      // Main style
      wp_enqueue_style('better-chat-support', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');

      // Main RTL styles
      if (is_rtl()) {
        wp_enqueue_style('better-chat-support-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), array(), self::$version, 'all');
      }
      ...
      wp_localize_script('better-chat-support', 'BetterChatSupport_vars', array(
        'color_palette'     => apply_filters('BetterChatSupport_color_palette', array()),
        'i18n'              => array(
          'confirm'         => esc_html__('Are you sure?', 'better-chat-support'),

          /* translators: %s: Minimum number of characters */
          'typing_text'     => esc_html__('Please enter %s or more characters', 'better-chat-support'),
          'searching_text'  => esc_html__('Searching...', 'better-chat-support'),
          'no_results_text' => esc_html__('No results found.', 'better-chat-support'),
        ),
      ));
      ...
      if (! empty(self::$fields)) {
        foreach (self::$fields as $field) {
          if (! empty($field['type'])) {
            $classname = 'BetterChatSupport_Field_' . $field['type'];
      ...
      do_action('BetterChatSupport_enqueue');
    }
    ...
    public static function add_typography_enqueue_styles()
    {
      ...
      if (! empty(self::$webfonts)) {
        ...
        if (! empty(self::$webfonts['enqueue'])) {
          ...
          wp_enqueue_style('better-chat-support-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), array(), null);
        }
        ...
        if (! empty(self::$webfonts['async'])) {
          ...
          wp_enqueue_script('better-chat-support-google-web-fonts', esc_url('//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'), array(), null);

          wp_localize_script('better-chat-support-google-web-fonts', 'WebFontConfig', array('google' => array('families' => $fonts)));
        }
      }
    }
    ...
    public static function add_admin_body_class($classes)
    {
      ...
      if (apply_filters('BetterChatSupport_fa4', false)) {
        $classes .= 'better-chat-support-fa5-shims';
      }
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      if (! empty($field['_notice'])) {
        ...
        $field            = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'better-chat-support') . ' <strong>(' . esc_html($field_type) . ')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      }
      ...
      $is_pseudo  = (! empty($field['pseudo'])) ? ' better-chat-support-pseudo-field' : '';
      ...
      if (! empty($field['dependency'])) {
        ...
        $visible = (! empty($depend_visible)) ? ' better-chat-support-depend-visible' : ' better-chat-support-depend-hidden';
      }

      // These attributes has been sanitized above.
      echo '<div class="better-chat-support-field better-chat-support-field-' . esc_attr($field_type . $is_pseudo . $class . $visible) . '"' . wp_kses_post($depend) . '>';
      ...
      if (! empty($field_type)) {
        if (! empty($field['title'])) {
          ...
          if (! empty($field['title_help']) || ! empty($field['title_video'])) {
            ...
            $title_help      = sprintf('<span class="better-chat-support-help better-chat-support-title-help">
							<span class="better-chat-support-help-text' . esc_attr($video_help_attr) . '">%s</span>
          ...
          echo '<div class="better-chat-support-title">';
          echo '<h4>' . wp_kses_post($field['title']) . $title_help . '</h4>'; // phpcs:ignore -- Attributes are escaped above.
          echo (! empty($field['subtitle'])) ? '<div class="better-chat-support-subtitle-text">' . wp_kses_post($field['subtitle']) . '</div>' : '';
          echo '</div>';
        }

        echo (! empty($field['title'])) ? '<div class="better-chat-support-fieldset">' : '';

        $value = (! isset($value) && isset($field['default'])) ? $field['default'] : $value;
        $value = (isset($field['value'])) ? $field['value'] : $value;

        $classname = 'BetterChatSupport_Field_' . $field_type;

        if (class_exists($classname)) {
          $instance = new $classname($field, $value, $unique, $where, $parent);
          $instance->render();
        } else {
          echo '<p>' . esc_html__('Field not found!', 'better-chat-support') . '</p>';
        }
      } else {
        echo '<p>' . esc_html__('Field not found!', 'better-chat-support') . '</p>';
      }
...
BetterChatSupport::init(__FILE__);
