 ...
 *
 */
if ( ! class_exists( 'BetterChatSupport_Field_link_color' ) ) {
  class BetterChatSupport_Field_link_color extends BetterChatSupport_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'better-chat-support' ),
        'hover'    => esc_html__( 'Hover', 'better-chat-support' ),
        'active'   => esc_html__( 'Active', 'better-chat-support' ),
        'visited'  => esc_html__( 'Visited', 'better-chat-support' ),
        'focus'    => esc_html__( 'Focus', 'better-chat-support' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="better-chat-support--left better-chat-support-field-color">';
          echo '<div class="better-chat-support--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="better-chat-support-color"'. wp_kses_post($default_attr) . wp_kses_post($this->field_attributes()) .'/>';
          echo '</div>';
