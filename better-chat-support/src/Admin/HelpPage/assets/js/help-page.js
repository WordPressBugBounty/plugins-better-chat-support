; (function ($) {
	...
	$(window).on('hashchange', function() {
		...
		if ('get-start' === hash || 'recommended' === hash || 'lite-to-pro' === hash || 'pro-plugins' === hash || '#tab=help' === window.location.hash) {
			$('.better-chat-support-nav.better-chat-support-nav-options ul li:last-child a').addClass('better-chat-support-active');
			$('.better-chat-support-section[data-section-id="help"]').show();
		}
		if( !hash && '#tab=help' !== window.location.hash ) {
			$('.better-chat-support-section[data-section-id="help"]').hide();
		}
	})
	// Help page tab menu script.
	$('.better-chat-support').on('click', '.header_nav a', function (e) {
	...
	if ('get-start' === hash) {
		$('.better-chat-support .header_nav a[data-id="get-start-tab"]').trigger('click');
	}

    if ('recommended' === hash) {
		$('.better-chat-support .header_nav a[data-id="recommended-tab"]').trigger('click');
	}
	if ('lite-to-pro' === hash) {
		$('.better-chat-support .header_nav a[data-id="lite-to-pro-tab"]').trigger('click');
	}
	if ('pro-plugins' === hash) {
		$('.better-chat-support .header_nav a[data-id="pro-plugins-tab"]').trigger('click');
	}
